...
### Key Points
- Primary key, SystemId, and data audit fields are always loaded automatically
- Fields that are filtered upon are always loaded automatically
- FlowFields and FlowFilters are not supported with SetLoadFields
- Avoid using SetLoadFields on records passed or received as `var` parameters for maintainability reasons (callers may expect all fields to be available). Exceptions may apply in performance-critical scenarios.

### Examples

```al
// Good example - Only load the field needed for calculation
procedure ComputeArithmeticMean(ItemCategoryCode: Code[20]): Decimal
var
    Item: Record Item;
    SumTotal: Decimal;
    Counter: Integer;

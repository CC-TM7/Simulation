...
## Rule 4: Generic Event Subscriber Pattern

### Intent
Event subscribers should be generic and reusable, matching the actual event name without specific business logic descriptions. The subscriber should only orchestrate calls to dedicated business logic procedures. This pattern ensures the event subscriber can handle multiple use cases in the future and maintains clean separation of concerns.

### Examples

```al
// Good example - Generic event subscriber with business logic separation
codeunit 50105 "CCO Sales Posting Handler"
{
    [EventSubscriber(ObjectType::Codeunit, Codeunit::"Sales-Post", OnBeforePostSalesDoc, '', false, false)]
    local procedure OnBeforePostSalesDoc(var SalesHeader: Record "Sales Header"; var IsHandled: Boolean)
    begin
        // Use event for to extend business logic 
        PerformExtensionLogic();
        // Future validations can be added here:
        PerformExtensionLogic2();
    end;
}

// Bad example - Specific event subscriber name
codeunit 50105 "CCO Sales Posting Handler"
{
    [EventSubscriber(ObjectType::Codeunit, Codeunit::"Sales-Post", OnBeforePostSalesDoc, '', false, false)]
    local procedure OnBeforePostExtensionLogic(var SalesHeader: Record "Sales Header"; var IsHandled: Boolean)
    begin
        // All business logic extension code mixed in event subscriber
    end;
}
```